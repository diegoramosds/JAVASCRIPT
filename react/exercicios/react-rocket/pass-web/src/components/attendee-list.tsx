   ...
   function goToFirstPage() {
    ...
    <div className='flex flex-col gap-4'>
        ...
        <Table>
            ...
            <tbody>
               {attendees.slice((page - 1) * 10, page * 10 ).map((atteendee) => {
                return(
                    <TableRow key={atteendee.id}>
                    <TableCell >
                        <input type="checkbox" className='sizee-4 bg-black/20 rounded border border-white/1'/>
                    </TableCell>
                    <TableCell >{atteendee.id}</TableCell>
                    <TableCell >
                        <div className='flex flex-col gap-1'>
                            <span className='font-semibold text-white'>{atteendee.name}</span>
                            <span>{atteendee.email}</span>
                        </div>
                        </TableCell>
                        <TableCell >{dayjs().to(atteendee.createdAt)}</TableCell>
                        <TableCell >{dayjs().to(atteendee.checkedInAt)}</TableCell>
                        <TableCell >
